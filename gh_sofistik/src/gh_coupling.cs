...
public class GH_CouplingStruc
{
   ...
   private Enum setInput(IGH_GeometricGoo gg, bool a)
   {
      Enum res = State.OK;
      bool isPoint = false;
   ...
   public List<Line> GetConnectionLines()
   {
      ...
      if (IsACurve && IsBCurve)
      {
         ...
         if (startPointA.DistanceTo(endPointB) < startPointA.DistanceTo(startPointB))
...
public class GH_Coupling : GH_GeometricGoo<GH_CouplingStruc>, IGH_PreviewData
{
   ...
   public void DrawViewportWires(GH_PreviewWireArgs args)
   {
      ...
      if (!DrawUtil.CheckSelection(col))
         ...
         _cplCond.DrawInfo(args.Pipeline, (GroupId == 0 ? "" : "Grp Id: " + GroupId + "\n") + "Fix: " + FixLiteral);
   ...
   private void updateCouplings() {
      ...
      _cplCond.CreateCouplingSymbols(Value.GetConnectionLines());
   }
